...
function buildOTPEmail(code, otpId) {
    var expirySeconds = parseInt(process.env.OTP_EXPIRY_SECONDS || '300', 10) || 300;
    var expiryMinutes = Math.round(expirySeconds / 60);
    return {
        // Unique subject reduces Gmail threading/forwarding grouping
        subject: `SkillBridge Verification Code: ${code} (${otpId})`,
        text: "Your verification code is: " + code + ". It expires in " + expiryMinutes + " minutes. OTP-ID: " + otpId,
        html: '<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;border:1px solid #e0e0e0;border-radius:10px">'
            ...
            + '</div>'
            + '<p style="font-size:14px;color:#718096;text-align:center">This code will expire in ' + expiryMinutes + ' minutes.</p>'
